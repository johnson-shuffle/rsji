...
These data are matched the spatial data above by incident datetime and type of crime which only works for instances in which this information is unique (otherwise it is not possible to match).  Using this strategy, approximately 14% of the records can be matched.  The signficance of this is that the matched data provide demographic information by both sector AND race. 

Aggregrate counts are summarized below:
...
Demographic data for each police sector are obtained by utilizing the 2015 five-year estimates of the American Community Survey.  These data are available at the block-group level of which the city contains 482; of these, 389 lie completely within one of the the 17 police sectors.  For those block-groups lying in multiple sectors, the percentage of area of overlap bewteen the sector and the block-group is used to spatially allocate the data.  For example, approximatley 30% of the area of Block Group 6 of Census Tract 32 is located in sector B and so sector B is apportioned 30% of that block-group's population, etc.

A natural point of comparison is the deomgraphic profile of the city as a whole.  This is summarized below:
...
In addition, two alternative specifications are considered.  First, the offset $a_{rs(t-1)}$ is modeled more flexibly by including it as a predictor with its own coefficent:
...
Two additional specifications of the model for $\theta_{rs}$ are also considered.  First, the population $p_{rs}$ is given its own coefficent:
...
Second, each sector's log median income $z_{2s}$ and proportion of the population with at least a four-year degree $z_{3s}$ are included as predictors:
...
where $\sigma_{\alpha}$, $\sigma_{\beta}$, and $\sigma_{\epsilon}$ represent the variation in the parameters for race, sector, and overdispersion respectively.  A similar set of priors are used for the approahces which explicitly model $\theta_{rs}$:
...
```{r rhat, fig.width = 5, fig.height = 4}
# dat <- map_dfr(m, extract_rhat)
# p <- ggplot(dat) +
#   geom_histogram(aes(rhat, colour = param, fill = model))
# p
```
...
dat <- tibble(
  model = m,
  a1 = 100 * tb_stops[, 'TRUE'] / 136,
  a2 = c(NA, NA, NA, 100 * tb_arrests[, 'TRUE'] / 136)
)
...
knitr::kable(
  ...
  digits = 1,
  caption = 'Classification rates (95% HPD intervals of the simulated model contain observed data)'
  )
```